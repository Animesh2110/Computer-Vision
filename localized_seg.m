...
function [seg,phi] = localized_seg(I,init_mask,max_its,alpha,beta,method,display,rad)
  ...
  if(~exist('rad','var')) 
   ...
   % if(display>0) 
   %   disp(['localization radius is: ' num2str(rad) ' pixels']); 
   % end
  end
...
%      disp('size of signum is');
%      disp(size(sign(phi0(idx))));
%      disp('size of min point is');
%      disp(size(GetminPoint(idx,idx1)));
    ...
    switch method  %-choose which energy is localized
     ...
     otherwise,     
       F = -(u-v).*(2.*I(idx)-u-v)+ beta*sign(phi0(idx)).*GetminPoint(idx,idx1).^2;
      %F = -((u-v).*((I(idx)-u)./Ain+(I(idx)-v)./Aout))+ 5*sign(phi0(idx)).*GetminPoint(idx,idx1).^2;
    end
...
%   contour(phi, [0 0], 'k','LineWidth',0.01);
  hold off;
  title([num2str(i) ' Iterations']); drawnow;
...
% Calculte minimum Distance
function t = GetminPoint(idx,idx1)
  N = 128;
  t = zeros(1,size(idx,2));
  for i = 1:size(idx,2)
      minimum_distance = 65536000;
      r = floor(idx(1,i)/N);
      c = rem(idx(1,i),N)  ;
    for j = 1:size(idx1,2)
      r1 = floor(idx1(1,j)/N);
      c1 = rem(idx1(1,j),N) ;
      distance = sqrt((r-r1)^2 + (c-c1)^2);
